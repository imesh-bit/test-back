    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter();  Connection connection = JDBC.con_payroll();) {
            if (jwt.validateToken(requestTokenHeader)) {
                ModelUser model = new ModelUser(connection);

                if (request.getParameter("getUserList") != null) {

                    response.setContentType("application/json");
                    response.getWriter().print(model.getUserList());

                }
                 if (request.getParameter("getSelectedUser") != null) {

                    int empId = Integer.parseInt(request.getParameter("empId"));

                    response.setContentType("application/json");
                    response.getWriter().print(model.getSelectedUser(empId));
                ...
                if (request.getParameter("saveUsers") != null) {
                    try {
                        ...
                        JSONObject jsonResponse = model.saveUser(requestJson, connection);
                        response.setContentType("application/json");
