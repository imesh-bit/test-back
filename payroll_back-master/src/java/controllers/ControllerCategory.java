    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter();  Connection connection = JDBC.con_payroll();) {
            if (jwt.validateToken(requestTokenHeader)) {
                ModelCategory model = new ModelCategory(connection);

                if (request.getParameter("getCategoryList") != null) {

                    response.setContentType("application/json");
                    response.getWriter().print(model.getCategoryList());

                }
                if (request.getParameter("saveCategory") != null) {
                    try {
                        ...
                        JSONObject jsonResponse = model.saveCategory(requestJson, connection);
                        response.setContentType("application/json");
                 ...
                 if (request.getParameter("deleteCategory") != null) {
                    try {
                        ...
                        JSONObject jsonResponse = model.deleteCategory(requestJson, connection);
                        response.setContentType("application/json");
                        out.print(jsonResponse);
